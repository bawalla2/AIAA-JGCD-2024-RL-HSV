...
for i = 1:m
    ...
    metr.varname = [currvarname 'min'];
    metr.texname = ['$' currtexname '_{min}$'];
    ...
    metr.varname = [currvarname 'max'];
    metr.texname = ['$' currtexname '_{max}$'];
    metr.format = format_default;
...
% Bool -- keeps track of if at least one of the presets has the current
% metric
hascurrmetric_atleast1 = logical(zeros(nummetric,1));
...
for presetcount = 1:numpresets
    ...
    for i = 1:nummetric
        ...
        % If current metric is in this group, set flag high
        if hascurrmetric
            hascurrmetric_atleast1(i) = 1;
        end
...
% ***********************
%       
% RETURN ONLY METRICS WHICH ARE PRESENT IN AT LEAST ONE GROUP
%

metricdatatmat = metricdatatmat(hascurrmetric_atleast1,:);
metricdata_cell = metricdata_cell(hascurrmetric_atleast1,:);
metric_cell = metric_cell(hascurrmetric_atleast1);