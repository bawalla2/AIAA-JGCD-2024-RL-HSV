...
% System tag, names
hassystag = isfield(master_settings,'systag');
if hassystag
...
for i = 1:numgroups
    ...
    istraining = groupi.istraining;

    % Check if is a plot group
    isplot = groupi.isplot;

    % If is not a training preset group, make textable
    if ~istraining && ~groupi.iseval && ~isplot
